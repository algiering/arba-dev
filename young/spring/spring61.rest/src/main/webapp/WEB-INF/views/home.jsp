<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>
<%@ page session="false"%>
<html>
<head>
<<<<<<< HEAD
<title>Home</title>
<script src="/resources/jquery-3.2.1.js"></script>
<script>
    $(document).ready(function(event) {
        $("#joinOk").click(function(e) {
            $.ajax({
                url : '/rest/getboardone',
                data : {
                    'cd' : 'free'
                }, // { 'data1':'test1', 'data2':'test2' }
                type : 'post', // get, post
                timeout : 30000, // 30 초
                dataType : 'json', // text, html, xml, json, jsonp, script
                beforeSend : function() {
                    // 통신이 시작되기 전에 이 함수를 타게 된다.
                    $('#message1').html('<img src="/resources/loading.gif">');
                }
            }).done(function(data, textStatus, xhr) {
                // 통신이 성공적으로 이루어졌을 때 이 함수를 타게 된다.
                if (!data) {
                    alert("존재하지 않는 boardcd 입니다");
                } else {
                    alert("존재하는 boardcd 입니다");
                    10
                }
            }).fail(function(xhr, textStatus, error) {
                // 통신이 실패했을 때 이 함수를 타게 된다.
                var msg = '';
                msg += "code:" + xhr.status + "\n";
                msg += "message:" + xhr.responseText + "\n";
                msg += "status:" + textStatus + "\n";
                msg += "error : " + error + "\n";

                console.log(msg);
            }).always(function(data, textStatus, xhr) {
                // 통신이 실패했어도 성공했어도 이 함수를 타게 된다.
                $('#message1').html('');
            });
        });

    })
</script>
=======
	<title>Home</title>
<<<<<<< HEAD
    <script src="/resources/jquery-3.2.1.js"></script>
    <script>
        $(document).ready( function(event){
        	
        	$("#joinOk").click( function(e){

        	    $.ajax({
        	        url : '/rest/getboardone',
        	        data: { 'cd' : 'free' }, // { 'data1':'test1', 'data2':'test2' }
        	        type: 'get',       // get, post
        	        timeout: 30000,    // 30초
        	        dataType: 'json',  // text, html, xml, json, jsonp, script
        	        beforeSend : function() {
        	            // 통신이 시작되기 전에 이 함수를 타게 된다.
        	            $('#message1').html('<img src="/resources/loading.gif">');
        	        }
        	    }).done( function(data, textStatus, xhr ){
        	        // 통신이 성공적으로 이루어졌을 때 이 함수를 타게 된다.
        	        if(!data){
        	            alert("존재하지 않는 boardcd  입니다");
        	        }
        	        else {
        	            alert("존재하는 boardcd  입니다");
        	        }
        	    }).fail( function(xhr, textStatus, error ) {
        	        // 통신이 실패했을 때 이 함수를 타게 된다.
        	        var msg ='';
        	        msg += "code:"    + xhr.status         + "\n";
        	        msg += "message:" + xhr.responseText   + "\n";
        	        msg += "status:"  + textStatus         + "\n";
        	        msg += "error  : "+ error              + "\n";
        	        
        	        console.log(msg);
        	    }).always( function(data, textStatus, xhr ) {
        	        // 통신이 실패했어도 성공했어도 이 함수를 타게 된다.
        	        $('#message1').html('');
        	    });

        	});

        });
    </script>
>>>>>>> fc9f1415ef71f0eb673a406c582c9897001e7bdf
=======
>>>>>>> 76874a403a6d22e5659a63aec79643b706324601
</head>
<body>
    <h1>Hello world!</h1>
<<<<<<< HEAD
    <P>The time on the server is ${serverTime}.</P>
=======

    <P>  The time on the server is ${serverTime}. </P>
>>>>>>> 4b6e1ed80aadf6810781b2c638e03dd1f6feaaf7
    <hr>

    <h3> select 실습</h3>
    <p> <a href="./rest/curtime"                           target="_blank"> /rest/curtime  </a></p>
    <p> <a href="./rest/checkid?id=id1"                    target="_blank"> /rest/checkid?id=id1  </a></p>
    <p> <a href="./rest/loginperson?id=id1&pw=pw1"         target="_blank"> /rest/loginperson?id=id1&pw=pw1  </a></p>
    <p> <a href="./rest/loginperson?id=testid&pw=testpw"   target="_blank"> /rest/loginperson?id=testid&pw=testpw  </a></p>
    <p> <a href="./rest/selectparam?name=name1"            target="_blank"> /rest/selectparam?name=name1    (GET)  </a></p>
    <p> <a href="./rest/selectmodel"                       target="_blank"> /rest/selectmodel             (FORM:POST)       </a></p>
    <p> <a href="./rest/selectjson"                        target="_blank"> /rest/selectjson              (JSON:POST)      </a></p>
    <p> <a href="./rest/selectmap"                         target="_blank"> /rest/selectmap               (JSON:POST)    </a></p>
    <br>
   
   
    <h4> 만들기 실습 : selectpaging 메서드를 만들어 보겠읍니다</h4>
    <p> <a href="./rest/selectpaging?start=10&end=20"      target="_blank"> /rest/selectpaging?start=100&end=200         </a></p>
    <br>
    

<<<<<<< HEAD
    <h1>ajax 통신</h1>
    <button id="joinOk">getboardone 호출</button>
    <div id="message1"></div>

    <h1>User</h1>
<<<<<<< HEAD
    <p>
        <a href="/rest/curtime" target="_blank">/rest/curtime</a>
    </p>
    <p>
        <a href="/rest/selectuserlist" target="_blank">/rest/selectuserlist</a>
    </p>
    <p>
        <a href="./rest/login?userid=test1id&passwd=test1pw"
            target="_blank">
            /rest/login?userid=test1id&passwd=test1pw </a>
    </p>
    <p>
        <a href="./rest/login?userid=userid&passwd=password"
            target="_blank">
            /rest/login?userid=userid&passwd=password </a>
    </p>
    <p>
        <a href="./rest/checkuserid?userid=userid" target="_blank">
            ./rest/checkuserid?userid=userid </a>
    </p>
    <p>
        <a href="./rest/checkuserid?userid=fadfad" target="_blank">
            ./rest/checkuserid?userid=fadfad </a>
    </p>
    <p>
        <a
            href="./rest/updatepasswd?userid=userid&currentPasswd=password&newPasswd=newpw"
            target="_blank">
            ./rest/updatepasswd?userid=userid&currentPasswd=password&newPasswd=newpw
        </a>
    </p>
    
    <p>
        <a href="./rest/insertone?id=3i&pw=3p&name=3n&email=3e"
            target="_blank"> /rest/insertone </a>
    </p>

    <p>@ModelAttribute 실습</p>

    
    <p>
        <a href="./rest/insertuser?userid=111&email=111" target="_blank">
            ./rest/insertuser?userid=111&email=111 </a>
    </p>
    <hr>
    <p>@RequestBody 실습 : json형태로 POST만 가능 soapui를 이용해 테스트</p>
    <a href="./rest/updateuserinfo" target="_blank">./rest/updateuserinfo</a>
    
    <hr>

    <h1>Board</h1>
    <p>
        <a href="/rest/getboardone?cd=free" target="_blank">
            /rest/getboardone?cd=free </a>
    </p>
    <p>
        <a href="/rest/getboardone" target="_blank">
            "/rest"/getboardone" </a>
    </p>
    <p>
        <a href="/rest/getboardname" target="_blank">
            "/rest/getboardname" </a>
    </p>
    <p>
        <a href="/rest/getboardtotalrecord" target="_blank">
            "/rest/getboardtotalrecord" </a>
    </p>
    <p>
        <a href="/rest/getboardpaging" target="_blank">
            "/rest/getboardpaging" </a>
    </p>
    <p>
        <a href="/rest/getboardlist" target="_blank">
            "/rest/getboardlist" </a>
    </p>
    <p>
        <a href="/rest/insertboard" target="_blank">
            "/rest/insertboard" </a>
    </p>
    <p>
        <a href="/rest/updateboard" target="_blank">
            "/rest/updateboard" </a>
    </p>
    <p>
        <a href="/rest/deleteboard" target="_blank">
            "/rest/deleteboard" </a>
    </p>
    <p>
        <a href="/rest/insertboardlist" target="_blank">
            "/rest/insertboardlist" </a>
    </p>
    <p>
        <a href="/rest/getarticletotalrecord" target="_blank">
            "/rest/getarticletotalrecord"</a>
    </p>
    <p>
        <a href="/rest/getarticlelist" target="_blank">
            "/rest/getarticlelist" </a>
    </p>
    <p>
        <a href="/rest/getarticle" target="_blank">
            "/rest/getarticle" </a>
    </p>
    <p>
        <a href="/rest/insertarticle" target="_blank">
            "/rest/insertarticle" </a>
    </p>
    <p>
        <a href="/rest/updatearticle" target="_blank">
            "/rest/updatearticle" </a>
    </p>
    <p>
        <a href="/rest/deletearticle" target="_blank">
            "/rest/deletearticle" </a>
    </p>
    <p>
        <a href="/rest/increasehit" target="_blank">
            "/rest/increasehit" </a>
    </p>
    <p>
        <a href="/rest/getnextarticle" target="_blank">
            "/rest/getnextarticle" </a>
    </p>
    <p>
        <a href="/rest/getprevarticle" target="_blank">
            "/rest/getprevarticle" </a>
    </p>
    <p>
        <a href="/rest/getattachfile" target="_blank">
            "/rest/getattachfile" </a>
    </p>
    <p>
        <a href="/rest/getattachfilelist" target="_blank">
            "/rest/getattachfilelist" </a>
    </p>
    <p>
        <a href="/rest/insertattachfile" target="_blank">
            "/rest/insertattachfile" </a>
    </p>
    <p>
        <a href="/rest/deleteattachfile" target="_blank">
            "/rest/deleteattachfile" </a>
    </p>
    <p>
        <a href="/rest/getcomment" target="_blank">
            "/rest/getcomment" </a>
    </p>
    <p>
        <a href="/rest/getcommentlist" target="_blank">
            "/rest/getcommentlist" </a>
    </p>
    <p>
        <a href="/rest/insertcomment" target="_blank">
            "/rest/insertcomment" </a>
    </p>
    <p>
        <a href="/rest/updatecomment" target="_blank">
            "/rest/updatecomment" </a>
    </p>
    <p>
        <a href="/rest/deletecomment" target="_blank">
            "/rest/deletecomment" </a>
    </p>

    <img
        src="data:image/png;base64,">
=======
    <p>  <a href="/rest/curtime" target="_blank">/rest/curtime</a>  </p>
    <p>  <a href="/rest/selectuserlist" target="_blank">/rest/selectuserlist</a>  </p>
    
    <p> @RequestParam 사용법 실습 </p>
    <p> <a href="./rest/login?userid=test1id&passwd=test1pw" target="_blank"> /rest/login?userid=test1id&passwd=test1pw  </a></p>
    <p> <a href="./rest/login?userid=userid&passwd=password" target="_blank"> /rest/login?userid=userid&passwd=password  </a></p>
    <p> <a href="./rest/checkuserid?userid=userid" target="_blank"> ./rest/checkuserid?userid=userid  </a></p>
    <p> <a href="./rest/checkuserid?userid=fadfad" target="_blank"> ./rest/checkuserid?userid=fadfad  </a></p>
    <p> <a href="./rest/updatepasswd?userid=userid&currentPasswd=password&newPasswd=newpw" target="_blank"> ./rest/updatepasswd?userid=userid&currentPasswd=password&newPasswd=newpw  </a></p>
    <p> <a href="./rest/insertone?id=3i&pw=3p&name=3n&email=3e" target="_blank"> /rest/insertone </a></p>
    
    <p> @ModelAttribute 사용법 실습 </p>
    <p> <a href="./rest/insertuser?userid=111&email=111" target="_blank"> ./rest/insertuser?userid=111&email=111 </a></p>    
    <hr>
    
    <p> @RequestBody 사용법 실습 : json 형태만 가능. POST만 가능. soapui를 이용해서 테스트 </p>
    <p> <a href="./rest/updateuserinfo" target="_blank"> ./rest/updateuserinfo</a></p>    
    <hr>
=======
    <h3> insert 실습</h3>
    <p> <a href="./rest/insertparam?name=inserttest"  target="_blank"> /rest/insertparam?name=inserttest (GET)    </a></p>
    <p> <a href="./rest/insertmodel"                  target="_blank"> /rest/insertmodel (POST) >>>> &lt;form /&gt;을 이용하여 모델 정보를 넘기는 경우   </a></p>
    <p> <a href="./rest/insertjsonobject"             target="_blank"> /rest/insertjsonobject (POST) >>>> 자바스크립트 객체(모델 정보)를 넘기는 경우   </a></p>
    <p> <a href="./rest/insertjsonarray"              target="_blank"> /rest/insertjsonarray (POST)  >>>> 자바스크립트 객체(모델 정보)를 배열로 넘기는 경우   </a></p>
    <br />

    <h3> update 실습</h3>
    <p> <a href="./rest/update"  target="_blank"> /rest/update (GET)    </a></p>
    <br />

    <h3> delete 실습</h3>
    <p> <a href="./rest/delete"  target="_blank"> /rest/delete (GET)    </a></p>
    <br />

    <hr />
>>>>>>> 4b6e1ed80aadf6810781b2c638e03dd1f6feaaf7

    <h1>Ajax로 REST 서비스 호출하기</h1>
    <p> 
        <a href="/rest/getboardone?cd=free"  target="_blank">  /rest/getboardone?cd=free   </a>
        <button id="joinOk">getboardone 호출</button>
        <div id="message1"></div>
    </p>

    <h1>Board REST 서비스</h1>
    <p> <a href="/rest/getboardone?cd=free"   target="_blank">  /rest/getboardone?cd=free   </a> </p>
    <p> <a href="/rest/getboardone"           target="_blank"> "/rest/getboardone"          </a> </p>
    <p> <a href="/rest/getboardname"          target="_blank"> "/rest/getboardname"         </a> </p>
    <p> <a href="/rest/getboardtotalrecord"   target="_blank"> "/rest/getboardtotalrecord"  </a> </p>
    <p> <a href="/rest/getboardpaging"        target="_blank"> "/rest/getboardpaging"       </a> </p>
    <p> <a href="/rest/getboardlist"          target="_blank"> "/rest/getboardlist"         </a> </p>
    <p> <a href="/rest/insertboard"           target="_blank"> "/rest/insertboard"          </a> </p>
    <p> <a href="/rest/updateboard"           target="_blank"> "/rest/updateboard"          </a> </p>
    <p> <a href="/rest/deleteboard"           target="_blank"> "/rest/deleteboard"          </a> </p>
    <p> <a href="/rest/insertboardlist"       target="_blank"> "/rest/insertboardlist"      </a> </p>
    <p> <a href="/rest/getarticletotalrecord" target="_blank"> "/rest/getarticletotalrecord"</a> </p>
    <p> <a href="/rest/getarticlelist"        target="_blank"> "/rest/getarticlelist"       </a> </p>
    <p> <a href="/rest/getarticle"            target="_blank"> "/rest/getarticle"           </a> </p>
    <p> <a href="/rest/insertarticle"         target="_blank"> "/rest/insertarticle"        </a> </p>
    <p> <a href="/rest/updatearticle"         target="_blank"> "/rest/updatearticle"        </a> </p>
    <p> <a href="/rest/deletearticle"         target="_blank"> "/rest/deletearticle"        </a> </p>
    <p> <a href="/rest/increasehit"           target="_blank"> "/rest/increasehit"          </a> </p>
    <p> <a href="/rest/getnextarticle"        target="_blank"> "/rest/getnextarticle"       </a> </p>
    <p> <a href="/rest/getprevarticle"        target="_blank"> "/rest/getprevarticle"       </a> </p>
    <p> <a href="/rest/getattachfile"         target="_blank"> "/rest/getattachfile"        </a> </p>
    <p> <a href="/rest/getattachfilelist"     target="_blank"> "/rest/getattachfilelist"    </a> </p>
    <p> <a href="/rest/insertattachfile"      target="_blank"> "/rest/insertattachfile"     </a> </p>
    <p> <a href="/rest/deleteattachfile"      target="_blank"> "/rest/deleteattachfile"     </a> </p>
    <p> <a href="/rest/getcomment"            target="_blank"> "/rest/getcomment"           </a> </p>
    <p> <a href="/rest/getcommentlist"        target="_blank"> "/rest/getcommentlist"       </a> </p>
    <p> <a href="/rest/insertcomment"         target="_blank"> "/rest/insertcomment"        </a> </p>
    <p> <a href="/rest/updatecomment"         target="_blank"> "/rest/updatecomment"        </a> </p>
    <p> <a href="/rest/deletecomment"         target="_blank"> "/rest/deletecomment"        </a> </p>
    
    <img src="data:image/png;base64," >
>>>>>>> fc9f1415ef71f0eb673a406c582c9897001e7bdf
</body>
</html>
